...
switch ( JRequest::getVar( 'task' ) )
{
	...
	//$mainframe->redirect( "index.php?task=showmodules&option=$option&limit=$limit&limitstart=$limitstart",$msg );
	break;
...
//echo $menu."hello";
		...
		for($i=0;$i<count($result);$i++) {
			...
			$swmenufree_array[] =array("TITLE" => $result2->title, "URL" =>  $result2->link , "ID" => $result2->id  , "PARENT" => $result2->parent_id ,  "ORDER" => $result2->lft, "TARGET" => $result2->browserNav,"ACCESS" => $result2->access );
...
function changeLanguage(){
	...
	if(fwrite ($handle, $lang)){
		$msg="Language has successfully been changed";
	}else{
	...
	header("location:index.php?option=com_swmenufree&task=upgrade");
