 ...
 * @since		1.5
 *
 * modified by reDim - Norbert Bayer
 *
 */
...
var ImageManager = this.ImageManager = {
	...
	onok: function()
	{
		...
		if (url != '') {
            ...
            if (xx > imagesizer2_x || yy > imagesizer2_y){

                var faktor = 0;

   				if (xx>yy || xx==yy){
   					faktor = img.width / imagesizer2_x ;
   				}

    			if (xx<yy){
   					faktor =  img.height / imagesizer2_y ;
   				}

                if (faktor>0){
                   var xx = Math.round( img.width / faktor , 0);
                   var yy = Math.round( img.height / faktor , 0);
                }
			...
			extra = extra + 'width="'+xx+'" ';
			extra = extra + 'height="'+yy+'" ';
