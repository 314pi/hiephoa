	...
	var nnScripts = new Class({
			...
			for (var i = 0; i < xml.getElementsByTagName('extension').length; i++) {
				...
				for (var j = 0; j < ext.childNodes.length; j++) {
					...
					if (node && node.firstChild) {
						el[node.nodeName] = String.from(node.firstChild.nodeValue).trim();
					}
				}
		...
		compareVersions: function(number1, number2)
		{
			if (( number1+'' ).indexOf('.') !== -1) {
				...
				number1 = ( number1[0]*1000000 )+( number1[1]*1000 )+( number1[2] );
			}
			if (( number2+'' ).indexOf('.') !== -1) {
				number2 = number2.split('.');
				number2 = ( number2[0]*1000000 )+( number2[1]*1000 )+( number2[2] );
			}
			if (number1 < number2) {
				return '<';
