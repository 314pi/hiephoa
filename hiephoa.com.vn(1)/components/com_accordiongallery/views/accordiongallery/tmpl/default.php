...
if(substr($jver, 0, 3) == '1.5') {
	...
	$params =  & $app->getParams('com_accordiongallery');
}
...
$slideshow_width                   = intval($params->get( 'gallery_width', 720 ));
...
$xml_fname    = 'accordiongallery';
$catppv_id = 'accordiongallery/xml/' . $xml_fname;
$module_path_swf = JPATH_SITE . DS . 'components'.DS.'com_accordiongallery'.DS.'sb'.DS.'modules'.DS;
$module_path = JPATH_SITE . DS . $pic_path . DS;
if (!is_dir($module_path . 'accordiongallery')) {
	mkdir($module_path . 'accordiongallery', 0777);
}
if (!is_dir($module_path . 'accordiongallery' . DS . 'xml')) {
	mkdir($module_path . 'accordiongallery' . DS . 'xml', 0777);
}


if (!function_exists('create_gallaccordioncp_xml_file')) {
function create_gallaccordioncp_xml_file($params, &$catppv_id)
{
$componentstr = &JComponentHelper::getComponent( 'com_accordiongallery' );
$params_comg = new JParameter($componentstr->params);
$pic_path = $params_comg->get('pic_path', 'images/accordiongallery/gallery');
...
if (!file_exists($module_path . $catppv_id . '.swf') ) {
	copy($module_path_swf . 'mod_accordion_gallery.swf', $module_path . $catppv_id . '.swf');
...
$defaultSelectedCategory = $params->get( 'default_category_id', '0' );
...
{
	$ret_arr = write_galaccordioncp_xml_data($curr_category->id, $curr_category->id, $params);
	if ($ret_arr['flag']) {
...
if (!function_exists('write_galaccordioncp_xml_data')) {
function write_galaccordioncp_xml_data($cat_name, $cat_id, $params)
{
...
$xml_data .= ' <element>';
				$xml_data .= ' <link target="'.trim($params->get( 'target', '_blank' )).'">'.trim($curr_product->linkit).'</link>';
...
create_gallaccordioncp_xml_file($params, $catppv_id);
?>
		<script src="<?php echo JURI::root()?>components/com_accordiongallery/js/swfobject.js" language="javascript"></script>
		<script type="text/javascript">
		var flashvars = {
			xmlPath: "<?php echo JURI::root() . $pic_path.'/'. $catppv_id; ?>.xml",
		...
		swfobject.embedSWF(
			"<?php echo JURI::root(). $pic_path.'/'.$catppv_id; ?>.swf", 
